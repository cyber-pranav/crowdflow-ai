...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 w-full z-50 flex justify-center items-center pb-[env(safe-area-inset-bottom)]">
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex flex-col items-center justify-center py-2 px-4 transition-all active:scale-90 duration-200 ease-out ${
                active
